...
(function( $ ) {
    ...
    $.fn.drawr = function( action, param ) {
        ...
        plugin.create_button = function(toolbox,type,data,css){
        	...
        	var showhide = "block";
        	var id= data.name;
        	if(data.icon == "mdi mdi-folder-open mdi-24px")
        	{
				id= "openfile";
				showhide = "none";
        	}
        	else if(data.icon == "mdi mdi-content-save mdi-24px"){
        		id= "savefile";
        	}
        	var self=this;
        	var el = $("<button style='float:left;display:"+showhide+";margin:0px;' id='" + id + "'><i class='" + data.icon + "'></i></button>");
		...
		this.each(function() {
			...
			if ( action === "start") {
				...
				//brush dialog
        		currentCanvas.$brushToolbox = plugin.create_toolbox.call(currentCanvas,"brush",{ left: $(currentCanvas).parent().offset().left, top: $(currentCanvas).parent().offset().top },"Tools",80);
